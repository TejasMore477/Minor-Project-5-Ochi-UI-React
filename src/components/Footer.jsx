...
function Footer() {
    ...
    const linksData = [
        {head:"S:",sublink:["Instagram","Behance","Facebook","Linked"]},
        {head:"L:",sublink:["202-1965 W 4th Ave","Vancouver,Canada","30 Chukarina St","Lviv, Ukraine"]},
        {head:"E:",sublink:["hello@ochi.design"]},
        {head:"M:",sublink:["Home","Services","Our work","About us","Insights","Contact us"]},
    ];

    const [link, setlinks] = useState(linksData);
  return (
    <div data-scroll data-scroll-section data-scroll-speed=".3" className='w-full h-screen grid md:grid-cols-2 gap-20 md:gap-5 md:px-16 px-5 md:pt-20 pt-5 pb-10'>
        <div className='md:w-full w-fit md:h-full h-[16vh] flex items-start justify-between flex-col'>
            <h1 className='font-title md:text-[6.3vw] text-[10vw] font-extrabold md:leading-[5vw] leading-[9vw]'>eye- <br />
            opening</h1>
            <svg width="64" height="30" viewBox="0 0 72 30" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className='md:w-full h-full flex flex-col justify-between'>
            <h1 className='font-title md:text-[6.3vw] text-[10vw] font-extrabold md:leading-[5vw]'>presentations</h1>
            <div className='w-full grid grid-cols-2 gap-10 px-6 py-3'>
                {link.map((elem, index)=>(
                    <div key={index} className='h-fit flex flex-col justify-between'>
                        <h2 className='mb-2 text-lg font-semibold'>{elem.head}</h2>
                        <div>
                            {elem.sublink.map((item, index)=>(
                                <p key={index} className='underline cursor-pointer'>{item}</p>
                            ))}
                        </div>
                    </div>
                ))}
